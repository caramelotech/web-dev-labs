...
Na prática, a saída mais comum não é perseguir exactly-once no broker, é aceitar **at-least-once** (que é mais simples e mais barato) e fazer o consumer tratar duplicatas do lado dele. Há duas ferramentas para isso, que se complementam: a [idempotência](/labs/web-dev/resiliencia/02-idempotencia/), que faz reprocessar a mesma mensagem produzir o mesmo resultado que processá-la uma vez, e a [deduplicação](/labs/web-dev/mensageria/07-deduplicacao-de-mensagens/), que reconhece a mensagem repetida pelo ID e a descarta antes de processar.
