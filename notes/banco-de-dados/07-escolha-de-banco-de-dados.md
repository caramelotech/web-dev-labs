...
As notas de [Teorema de CAP](/labs/web-dev/banco-de-dados/05-teorema-de-cap/) e [Teorema de PACELC](/labs/web-dev/banco-de-dados/06-teorema-de-pacelc/) explicam a teoria: todo banco distribuído troca consistência por disponibilidade (durante uma partição) e consistência por latência (no dia a dia). Mas na hora de escolher uma tecnologia de verdade, a pergunta muda de forma: não é "o que é CP?", é "qual desses bancos eu uso pro meu problema?".
...
- **Classificação PACELC:** a combinação (ex: `AP/EL`, `CP/EC`) explicada na nota de [PACELC](/labs/web-dev/banco-de-dados/06-teorema-de-pacelc/). Resume se o banco prioriza disponibilidade/latência ou consistência.
- **Tipo de banco:** o modelo de dados (colunar, documento, chave-valor, relacional). Quando o tipo vem marcado com **(ACID)**, é porque esse banco também oferece as garantias de [ACID](/labs/web-dev/banco-de-dados/04-acid/) em transações, além da classificação PACELC.
...
- **Replicação baseada em WAL:** o PostgreSQL replica dados enviando o _write-ahead log_ (o mesmo log usado para garantir durabilidade, ver [ACID](/labs/web-dev/banco-de-dados/04-acid/)) para as réplicas, que o reproduzem para chegar no mesmo estado.
